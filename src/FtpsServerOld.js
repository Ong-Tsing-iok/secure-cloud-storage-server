...
process.env.KEY_FILE = process.env.KEY_FILE || 'server.key'
process.env.CERT_FILE = process.env.CERT_FILE || 'server.crt'
process.env.CA_FILES = process.env.CA_FILES || ''
...
server = new FtpServer(options.host, {
  ...
  getRoot: function (connection) {
    return '/' + path.join(__dirname, __upload_dir, connection.username)
  },
